...
project = 'revision_stripe_bins_v1';
...
if savio
    ...
    out_prefix = '../../out/revisions_final/';
...
for i = 1:length(trace_struct_final)
    ...
    if sum(time_ft) >= minDP
        ...
        temp.Stripe = mode(round(trace_struct_final(i).mike_stripe_id(time_ft),1));                           
...
rng('shuffle')
inference_list = randsample(1:numel(stripe_index),numel(stripe_index),false);
for s = 1:numel(stripe_index)
    stripe_bin = stripe_index(s); % get groups for present iteration           
    for b = 1:n_bootstrap                
        local_struct = struct;
        init_struct = struct;
        output = struct;                
        ...
        fName_sub = ['eveSet_w' num2str(w) '_K' num2str(K) ...
            '_stripe' num2str(stripe_bin) '_t' inference_id];                
        out_file = [out_dir '/' fName_sub];             
               
        trace_filter = ismember(stripe_id_vec,stripe_bin);        
